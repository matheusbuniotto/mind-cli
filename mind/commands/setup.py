...
def init():
    ...
    if cfg.get("api_key"):
        ...
        c.print(
            "mind never stores keys in config.yml — use shell exports and/or "
            "[bold]~/.mind/.env[/bold].\n"
        )
    ...
    new_cfg = {
        "api_key_source": existing.get("api_key_source", "env_first"),
        "base_url": base_url,
        "model": model,
    ...
    if found:
        ...
        if Confirm.ask("  Sync them all now? (requires API key in env)", default=False):
            ...
            for cwd in found[:10]:
                c.print(f"\n  → {cwd}")
                try:
                    sync.full_sync(cwd, progress=lambda m: c.print(f"    [dim]{m}[/dim]"))
